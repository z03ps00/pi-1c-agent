...
**Classify success phrases before error stems** — the platform reports success with the same words (`Ошибок не обнаружено`, `Предупреждений: 0`), so a bare "contains `Ошибка` / `Error`" test flags a clean run as broken and starts a fix loop against working code. Order and full pattern list — `$PI_CODING_AGENT_DIR/rules-1c/rules/designer-batch-checks.md → The success-phrase trap`. Add **`/DumpResult '{RESULT_PATH}'`** next to `/Out` in both command lines above: it writes the batch result as a number (`0` = success) and is the cheapest of the three signals to read (same rule, *The verdict is three signals*). Delete a stale result file before the run.
