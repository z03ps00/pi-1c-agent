...
Once installed, the project's EDT branch of the ruleset applies: `C:/DevopsMoments/pi-agents/config-1c/rules-1c/rules/edt-workflow.md` (process — source format, model↔disk, validation, deployment owner) and `C:/DevopsMoments/pi-agents/config-1c/skills/mcp-1c-tools/docs/edt-mcp.md` (server catalog and routing).
...
5. Confirm `.dev.env` holds `USE_EDT=true` (step 0) and tell the user in one line that the EDT rules are now active — `C:/DevopsMoments/pi-agents/config-1c/rules-1c/rules/edt-workflow.md`.
