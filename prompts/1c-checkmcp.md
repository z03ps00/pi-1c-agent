...
**`1C-docs-mcp` partial exposure — report as WARN.** If `docsearch` / `docinfo` are visible but **`standards` is not**, the server is running an image that predates the collection tools. The routed rules of `C:/DevopsMoments/pi-agents/config-1c/rules-1c/rules/` (`anti-patterns`, `dev-standards-architecture`, `dev-standards-code-style`, …) carry headings without bodies and cannot be retrieved on this image — report it, name `standards` as the missing tool, and recommend pulling a current `comol/1c_help_mcp`. Do **not** suggest reaching the standards through `docsearch`: that collection is not reachable from it, and no tool of this server accepts a `corpus` argument (`C:/DevopsMoments/pi-agents/config-1c/rules-1c/rules/help-corpus-retrieval.md`).
