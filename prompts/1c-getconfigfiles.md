...
The full procedure — parameters, `repoobjects.txt`, the `ibcmd` / Designer tool selection, the PowerShell templates, and the log check — is owned by the rule **`getconfigfiles.md`**. Read it from the canonical rules directory (source: `C:/DevopsMoments/pi-agents/config-1c/rules-1c/rules/getconfigfiles.md`) and follow it exactly; do not improvise flags that are not in the rule.
...
- **EDT projects** (`.dev.env` `USE_EDT=true`): the export is a Designer XML dump. If the working tree is an EDT (`src/**/*.mdo`) workspace, export to a separate directory and never mix the two trees — `C:/DevopsMoments/pi-agents/config-1c/rules-1c/rules/edt-workflow.md`.
