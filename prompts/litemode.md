---
description: [settings] Toggle lightweight QA mode — set VERIFICATION_DEPTH (full|standard|lite) in .dev.env and, at level lite, disable UI testing (UI_TESTING=off)
---

# /litemode — lightweight QA mode

Toggle how much static verification and UI testing the agent runs for the **project** by writing `VERIFICATION_DEPTH` (and, coupled, `UI_TESTING`) in `.dev.env`. Canonical behaviour of the depth levels — `$PI_CODING_AGENT_DIR/rules-1c/rules/verification-policy.md → "Verification depth levels"` and `$PI_CODING_AGENT_DIR/rules-1c/rules/dev-standards-env.md §1` (installed copy; match by file name per the path convention in `AGENTS.md`). Load `verification-policy.md` before acting.
