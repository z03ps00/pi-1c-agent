...
Default output is `build/{cf,cfe,epf,erf}` (sibling of `src/`). Create missing directories. Keep `build/` in `.gitignore`. If `.dev.env` `RELEASE_PATH` is set, use `{RELEASE_PATH}/{cf,cfe,epf,erf}` instead.

File names: **original name + date stamp** `OriginalName_YYYYMMDD.<ext>`. If that file already exists, use `OriginalName_YYYYMMDD-HHmmss.<ext>`. Do not overwrite.

Then, via the `db-ops` scripts (the 1C-infobase-operations hard gate — `AGENTS.md → Skills and Subagents`):

- main configuration: `db-dump-cf.ps1 -OutputFile "build/cf/<ConfigName>_YYYYMMDD.cf"`;
- each extension from `EXTENSION_NAMES`, in order: `db-dump-cf.ps1 -Extension <Name> -OutputFile "build/cfe/<Name>_YYYYMMDD.cfe"`;
- external processors from `src/epf` when the user asked to compile them: `build/epf/<Name>_YYYYMMDD.epf`;
- external reports from `src/erf`: `build/erf/<Name>_YYYYMMDD.erf`.

Verify each file exists and is non-empty; check the logs per the retry discipline of `$PI_CODING_AGENT_DIR/skills/1c-metadata-manage/docs/db-manage.md`.
