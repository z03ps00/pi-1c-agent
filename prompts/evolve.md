---
description: [maintainer] Aggregate accumulated rule-friction signals into user-approved behavior rules in LLM-RULES.md (ruleset self-improvement loop)
---
...
- **Upstream hint:** if an approved rule is not project-specific (it would improve the base ruleset for every project), say so — the fix belongs in the `1c-rules` source repo (`AGENTS.md` / `$PI_CODING_AGENT_DIR/rules-1c/rules/**`) through its normal change process; `LLM-RULES.md` stays the project-local layer either way.
...
- The command writes **only** `LLM-RULES.md` (plus the `memory.md` migration and `remember` notes described above) — never `AGENTS.md`, `USER-RULES.md`, `$PI_CODING_AGENT_DIR/rules-1c/rules/**`, installed rule copies, or `.dev.env`.
