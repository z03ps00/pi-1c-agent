...
- The command asks the user only when the input is unsupported or ambiguous; a recognised name is resolved silently. Asking inside this explicit configuration flow is fine — the never-ask policy of `$PI_CODING_AGENT_DIR/rules-1c/rules/dev-standards-env.md` applies to regular tasks, not to an invoked configuration command.
