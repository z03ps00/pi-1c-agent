...
Scenario set: taken from the argument / user request. Do not invent scenarios beyond what was asked; one scenario per user-visible behavior, in the template of `C:/DevopsMoments/pi-agents/config-1c/agents/1c-tester.md → Test Scenarios`.

**EDT projects** (`.dev.env` `USE_EDT=true`): the deploy step keeps a single owner per `C:/DevopsMoments/pi-agents/config-1c/rules-1c/rules/edt-workflow.md`.
