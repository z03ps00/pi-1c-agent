...
Ask first whether the user purchased the 1C MCP bundle and wants it installed. Do not pull or `docker run` before that consent.
...
Look up Tilda credentials only in local secret files the user already uses (installer `config.env` outside git, or project `.dev.env` keys the user named). **Never** read or write them in `memory.md`, Cognee, OpenViking, AGENTS, or handoffs.

If absent — ask the user in chat, in a single message:

> Для скачивания дистрибутива MCP нужно войти в личный кабинет `https://vibecoding1c.ru/`. Введите email (логин) и пароль. Сохранить их только в локальный секретный файл (`config.env` / `.dev.env`), не в `memory.md` и не через `remember`.

If the user agrees to save — write **only** to the local secret file they named. Never `memory.md`, never `remember`. If declined — keep credentials only for the current session and do not echo them back.
...
if ($loginResp.status -ne 'ok') {
    throw "Tilda login failed: status=$($loginResp.status), code=$($loginResp.code). Drop the cached credentials in the local secret file and ask the user again, or fall back to step 1.1.f (browser path)."
}
...
##### Step 1.1.g — Save credentials (local secret file only)

If the user agreed, persist login (not password if they refused) only in installer `config.env` or `.dev.env` outside git. Never append to `memory.md`. Never call `remember` with the password. Never echo the password in chat.
