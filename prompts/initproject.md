---
description: [settings] Alias of /init from-infobase — dump from an existing IB / .cf / .dt into a git repository
---

# /initproject — alias of /init from-infobase

Alias of `/init` from-infobase. State that once, then jump to the dump scenario (do not ask the empty-scaffold question).
...
**Shared values from sibling projects.** Before asking blocking keys, scan **one directory up** for similar 1C projects and propose non-secret shared values (`PLATFORM_PATH`, `PREFIX`, `DEVELOPER`, `COMPANY`, …) with source folder names. Wait for accept or corrections. Do not copy secrets or `INFOBASE_PATH` from neighbors. Then ask remaining blocking keys one by one; even autodetected values need confirmation before write.
...
Run the `/loadfrom1cbase` procedure (`$PI_CODING_AGENT_DIR/prompts/loadfrom1cbase.md` — single source of truth for the dump command lines, tool selection ibcmd-vs-Designer, and the result check) in **full-snapshot mode** (`/loadfrom1cbase all`): main configuration into `{EXPORT_PATH}`, then each extension from `EXTENSION_NAMES` into `{EXTENSIONS_PATH}\<Name>\`. Its dirty-working-tree guard applies unchanged.
...
2. Ensure `.gitignore` exists and covers at least `.dev.env` and `build/` (compiled binaries). Extend an existing `.gitignore`, never overwrite it. Also create `docs/` and `docs/techtask/` if missing (tracked in git; raw TZs go in `techtask`).
