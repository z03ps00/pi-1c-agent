...
- Containment is anchored at **`-SrcDir`**, the one path you vouched for: every component between it and a file the run would touch — the object directory, `Forms`, the descriptor, the form directory — must be a real directory entry. A symlink or Windows junction *anywhere* on that chain is refused with exit **2**, in all three modes (`-DryRun`, without `-Force`, with `-Force`). Checking a path only against its immediate parent is not enough: when the parent is itself a link, both sides resolve into the same foreign directory and the check passes.
...
- The mutation is one **bounded transaction**. The parent XML is parsed and serialized before the tree is touched; the deletions are renames into a `.remove-form-quarantine` directory next to `SrcDir` on the same filesystem, so nothing is destroyed until the whole operation succeeds, and every step has an undo. A failure at any step restores the original bytes and the original existence of the root XML, the form metadata and the form directory. Discarding the quarantine is **not** one of the undo steps: it is removed only after a rollback has verifiably put every payload back, or after the whole transaction has committed. If any file could not be restored, the quarantine **survives** — the run prints the primary failure first, then every kept copy and the path it belongs at, because at that point the quarantine is the only copy of those files there is.
