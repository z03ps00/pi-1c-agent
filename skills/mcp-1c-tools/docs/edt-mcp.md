...
`search_in_code` is a literal / regex textual sweep and is **not** ru/en dialect aware — it misses the other BSL language variant. It does not satisfy the MCP-first search chain (`C:/DevopsMoments/pi-agents/config-1c/rules-1c/rules/mcp-first-search.md`); use the indexed servers for search and this tool for targeted textual confirmation.
...
- **The model is the authority, the disk lags.** `resync_to_disk` flushes model → disk and reports desync. Reconcile before mixing EDT-MCP with file-level tools, and never alternate owners on one artifact — canon: `C:/DevopsMoments/pi-agents/config-1c/rules-1c/rules/edt-workflow.md → Model vs disk — the synchronization rule`.
