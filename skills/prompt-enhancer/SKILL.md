...
- The skill assumes no domain knowledge beyond the source. If 1C-specifics need clarification, consult the relevant on-demand rules in this repo (e.g. `$PI_CODING_AGENT_DIR/rules-1c/rules/coding-standards.md`, `$PI_CODING_AGENT_DIR/rules-1c/rules/dev-standards-architecture.md`) and the `1c-metadata-manage` skill (`docs/query-writing.md`, `docs/query-optimization.md`) for query and metadata details.
