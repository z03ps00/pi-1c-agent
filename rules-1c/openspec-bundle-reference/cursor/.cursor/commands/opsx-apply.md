...
The `## Genuine blockers` block in the opening message lists only blockers that pass this filter; if every candidate falls into the banned list above, the block is empty and the agent proceeds straight to implementation. See `C:/DevopsMoments/pi-agents/config-1c/rules-1c/rules/sdd-integrations.md → Apply-phase clarification discipline → Banned questions at apply time` for the full rule and rationale.
